// Add diagnostic logging at the global level
console.log('====== DASHBOARD.JS LOADED ======');

// Debug socket.io availability
if (typeof io !== 'undefined') {
    console.log('Socket.IO is available globally');
} else {
    console.error('Socket.IO is NOT available - check script loading');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const templateElement = document.getElementById('topic-card-template');
    console.log('Template HTML:', templateElement ? templateElement.outerHTML : 'NOT FOUND');
    ...
    try {
        ...
        socket.on('connect', () => {
            ...
            // Debug: list all methods on the socket object
            console.log('Socket methods available:', 
                Object.getOwnPropertyNames(
                    Object.getPrototypeOf(socket)
                ).filter(p => typeof socket[p] === 'function')
            );
        });
        ...
        console.log('Topic card template content:', topicCardTemplate ? (topicCardTemplate.content ? 'Has content' : 'No content') : 'N/A');
        ...
        function createDynamicTopicCard(topic, message) {
            ...
            if (!dynamicTopicCardsContainer) {
                console.error('Cannot create card: Missing container. Container:', !!dynamicTopicCardsContainer);
                return;
            }

            // Instead of using the template element, create the card directly
            // This approach is more compatible across browsers
            const cardHtml = `
                <div class="col-md-4 mb-3 dynamic-topic-card" id="topic-card-${topic.replace(/[^a-zA-Z0-9]/g, '-')}">
                    <div class="card shadow-sm h-100">
                        <div class="card-header text-white ${getHeaderColorClass(topic)}">
                            <i class="${getTopicIcon(topic)} me-2"></i><span class="topic-title">${getTopicTitle(topic)}</span>
                        </div>
                        <div class="card-body">
                            <div class="topic-content"></div>
                            <div class="mt-3">
                                <button class="btn btn-sm btn-outline-secondary history-button" type="button" 
                                        data-bs-toggle="collapse" 
                                        data-bs-target="#history-collapse-${topic.replace(/[^a-zA-Z0-9]/g, '-')}" 
                                        aria-expanded="false" 
                                        aria-controls="history-collapse-${topic.replace(/[^a-zA-Z0-9]/g, '-')}">
                                    <i class="fas fa-history me-1"></i>History
                                </button>
                                <div class="collapse mt-2 history-collapse" id="history-collapse-${topic.replace(/[^a-zA-Z0-9]/g, '-')}">
                        ...
                        <div class="card-footer text-muted small">
                            <i class="fas fa-tag me-1"></i><span class="topic-path">${topic}</span>
                        </div>
                    </div>
                </div>
            `;
            ...
            dynamicTopicCardsContainer.innerHTML += cardHtml;
            
            // Get the newly created card
            const card = document.getElementById(`topic-card-${topic.replace(/[^a-zA-Z0-9]/g, '-')}`);
            
            if (!card) {
                console.error(`Failed to find the created card for topic: ${topic}`);
                return;
            }
        ...
        // Helper function to get card header color class based on topic
        function getHeaderColorClass(topic) {
        ...
        // Helper function to get icon class based on topic
        ...
        function updateDynamicTopicCard(topic, message) {
            ...
            if (!contentEl) {
                console.error('Topic content element not found in card:', topic);
                return;
            }
